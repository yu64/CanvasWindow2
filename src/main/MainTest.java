...
import main.state.StateImpl;
import main.state.StateTable;
import main.util.TransformUtil;
import main.value.KeyFlags;
import main.value.LoaderManager;
import main.value.ReadTicket;
import main.value.test.TestObject1;
import main.value.test.TestObject2;
import main.value.test.TestReader2;
import main.view.scene.Node;
import main.view.scene.SubFrameNode;
...
public class MainTest {
	...
	public static void testMove(App app, Node node)
	{
		...
		app.getLogic().add(tpf -> {
			...
			if(flag.isTrue(KeyEvent.VK_A))
			{
				x += speed;
			}

			if(flag.isTrue(KeyEvent.VK_D))
			{
				x += -speed;
			}

			if(flag.isTrue(KeyEvent.VK_W))
			{
				y += speed;
			}

			if(flag.isTrue(KeyEvent.VK_S))
			{
				y += -speed;
			}
	...
	public static void testState(App app, Node node)
	{
		...
		table.register(stateA, stateB, () -> flag.isTrue(KeyEvent.VK_1));
		table.register(stateB, stateC, () -> flag.isTrue(KeyEvent.VK_2));
		table.register(stateC, stateA, () -> flag.isTrue(KeyEvent.VK_3));
		...
		node.add(g2 -> {
			...
			{
				g2.drawString("stateA", 50, 50);
			}

			if(table.getState() == stateB)
			{
				g2.drawString("stateB", 50, 50);
			}

			if(table.getState() == stateC)
			{
				g2.drawString("stateC", 50, 50);
			}
